...
public class SwiftFlutterBarcodeScanPlugin: NSObject, FlutterPlugin, ScanBarcodeDelegate,FlutterStreamHandler {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = SwiftFlutterBarcodeScanPlugin()
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        SwiftFlutterBarcodeScanPlugin.barcodeStream = events
        return nil
    }
    
    public func onCancel(withArguments arguments: Any?) -> FlutterError? {
        SwiftFlutterBarcodeScanPlugin.barcodeStream=nil
        return nil
    }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if let colorCode = args["lineColor"] as? String{
            SwiftFlutterBarcodeScanPlugin.lineColor = colorCode
        }else {
            SwiftFlutterBarcodeScanPlugin.lineColor = "#ff6666"
        }
        if let buttonText = args["cancelButtonText"] as? String{
            SwiftFlutterBarcodeScanPlugin.cancelButtonText = buttonText
        }else {
            SwiftFlutterBarcodeScanPlugin.cancelButtonText = "Cancel"
        }
        if let flashStatus = args["isShowFlashIcon"] as? Bool{
            SwiftFlutterBarcodeScanPlugin.isShowFlashIcon = flashStatus
        }else {
            SwiftFlutterBarcodeScanPlugin.isShowFlashIcon = false
        }
        if let isContinuousScan = args["isContinuousScan"] as? Bool{
            SwiftFlutterBarcodeScanPlugin.isContinuousScan = isContinuousScan
        }else {
            SwiftFlutterBarcodeScanPlugin.isContinuousScan = false
        }
        
        if let scanModeReceived = args["scanMode"] as? Int {
            if scanModeReceived == ScanMode.DEFAULT.index {
                SwiftFlutterBarcodeScanPlugin.scanMode = ScanMode.QR.index
            }else{
                SwiftFlutterBarcodeScanPlugin.scanMode = scanModeReceived
            }
        }else{
            SwiftFlutterBarcodeScanPlugin.scanMode = ScanMode.QR.index
        }
        ...
        if checkCameraAvailability(){
            if checkForCameraPermission() {
                SwiftFlutterBarcodeScanPlugin.viewController.present(controller
                                                                        , animated: true) {
                ...
                AVCaptureDevice.requestAccess(for: .video) { success in
                    DispatchQueue.main.async {
                        if success {
                            ...
                            SwiftFlutterBarcodeScanPlugin.viewController.present(alert, animated: true)
                        }
                    }
                }}
        }else {
    ...
    func showAlertDialog(title:String,message:String){
        ...
        SwiftFlutterBarcodeScanPlugin.viewController.present(alertController, animated: true, completion: nil)
    }
}
...
class BarcodeScannerViewController: UIViewController {
    ...
    private lazy var flashIcon : UIButton! = {
        ...
        flashButton.setImage(UIImage(named: "ic_flash_off", in: Bundle(for: SwiftFlutterBarcodeScanPlugin.self), compatibleWith: nil),for:.normal)
    ...
    private lazy var switchCameraButton : UIButton! = {
        ...
        button.setImage(UIImage(named: "ic_switch_camera", in: Bundle(for: SwiftFlutterBarcodeScanPlugin.self), compatibleWith: nil),for: .normal)
    ...
    public lazy var cancelButton: UIButton! = {
        let view = UIButton()
        view.setTitle(SwiftFlutterBarcodeScanPlugin.cancelButtonText, for: .normal)
        view.translatesAutoresizingMaskIntoConstraints = false
    ...
    func initUIComponents(){
        if isOrientationPortrait {
            screenHeight = (CGFloat)((SwiftFlutterBarcodeScanPlugin.scanMode == ScanMode.QR.index) ? (screenSize.width * 0.8) : (screenSize.width * 0.5))
            
        } else {
            screenHeight = (CGFloat)((SwiftFlutterBarcodeScanPlugin.scanMode == ScanMode.QR.index) ? (screenSize.height * 0.6) : (screenSize.height * 0.5))
        }
    ...
    func drawUIOverlays(withCompletion processCompletionCallback: () -> Void){
        ...
        if let qrCodeFrameView = qrCodeFrameView {
            ...
            if(!SwiftFlutterBarcodeScanPlugin.isShowFlashIcon){
    ...
    private func flashIconOff() {
        flashIcon.setImage(UIImage(named: "ic_flash_off", in: Bundle(for: SwiftFlutterBarcodeScanPlugin.self), compatibleWith: nil),for:.normal)
    }
    
    private func flashIconOn() {
        flashIcon.setImage(UIImage(named: "ic_flash_on", in: Bundle(for: SwiftFlutterBarcodeScanPlugin.self), compatibleWith: nil),for:.normal)
    }
    ...
    @IBAction private func cancelButtonClicked() {
        if SwiftFlutterBarcodeScanPlugin.isContinuousScan{
            self.dismiss(animated: true, completion: {
                SwiftFlutterBarcodeScanPlugin.onBarcodeScanReceiver(barcode: "-1")
            })
        }else{
    ...
    private func drawLine() {
        ...
        scanLine.backgroundColor = hexStringToUIColor(hex: SwiftFlutterBarcodeScanPlugin.lineColor)
        ...
        if SwiftFlutterBarcodeScanPlugin.scanMode == ScanMode.QR.index {
    ...
    public func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if supportedCodeTypes.contains(metadataObj.type) {
            ...
            if metadataObj.stringValue != nil {
                if(SwiftFlutterBarcodeScanPlugin.isContinuousScan){
                    SwiftFlutterBarcodeScanPlugin.onBarcodeScanReceiver(barcode: metadataObj.stringValue!)
                }else{
...
extension BarcodeScannerViewController{
    ...
    func updateUIAfterRotation(){
        DispatchQueue.main.async {
            ...
            if UIDevice.current.orientation == .portrait || UIDevice.current.orientation == .portraitUpsideDown {
                self.screenHeight = (CGFloat)((SwiftFlutterBarcodeScanPlugin.scanMode == ScanMode.QR.index) ? (self.screenSize.width * 0.8) : (self.screenSize.width * 0.5))
                
            } else {
                self.screenHeight = (CGFloat)((SwiftFlutterBarcodeScanPlugin.scanMode == ScanMode.QR.index) ? (self.screenSize.height * 0.6) : (self.screenSize.height * 0.5))
            }
